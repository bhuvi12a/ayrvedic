...
import { FaEnvelope, FaNewspaper, FaTachometerAlt, FaCalendarCheck, FaBoxOpen, FaClipboardList, FaTimes, FaWarehouse } from 'react-icons/fa';
...
export default function DashboardSidebar({ isOpen, onToggle }: DashboardSidebarProps) {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-0 left-0 h-full w-64 sm:w-72 lg:w-64 bg-white border-r border-gray-200 z-50 transform transition-transform duration-300 ease-in-out overflow-y-auto lg:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
          }`}
      >
        <div className="p-4 sm:p-6">
          ...
          <div className="mb-6 sm:mb-8 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-600 mt-1">Traditional Siddha Ayurveda Clinic</p>
            </div>
            <button
              onClick={onToggle}
            ...
            {menuItems.filter(item => item.section === 'main').map((item) => {
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2.5 sm:py-3 rounded-lg transition-all duration-200 ${isActive
              ...
              {menuItems.filter(item => item.section === 'enquiries').map((item) => {
                ...
                return (
                  <Link
                    ...
                    href={item.path}
                    onClick={() => window.innerWidth < 1024 && onToggle()}
                    className={`flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2.5 sm:py-3 mb-1 sm:mb-2 rounded-lg transition-all duration-200 ${isActive
                        ? 'bg-linear-to-r from-green-500 to-blue-600 text-white border border-green-600 shadow-lg'
                        : 'text-gray-700 hover:bg-gray-100 hover:text-gray-900'
                      }`}
                  >
